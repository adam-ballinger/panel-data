class Series {
    ...
    _initializeData(data) {
      if (Array.isArray(data)) {
        if (data.every(Number.isInteger)) return new Int32Array(data);
        if (data.every(v => typeof v === 'number')) return new Float64Array(data);
    ...
    _inferDtype(data) {
        ...
        if (Array.isArray(data) && data.every(Number.isInteger)) return 'int32';
        
        // Check if all elements are numbers
        if (Array.isArray(data) && data.every(v => typeof v === 'number')) return 'float64';
        
        // Check if the data is a string array (object dtype)
        if (Array.isArray(data) && data.every(v => typeof v === 'string')) return 'object';
